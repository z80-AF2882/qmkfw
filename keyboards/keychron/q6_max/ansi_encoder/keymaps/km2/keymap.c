...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [WIN_BASE] = LAYOUT_109_ansi(
        KC_ESC,   KC_F1,    KC_F2,    KC_F3,    KC_F4,    KC_F5,    KC_F6,    KC_F7,    KC_F8,    KC_F9,    KC_F10,   KC_F11,   KC_F12,     KC_MUTE,    KC_PSCR,  MY_DBG1,  MY_DBG2,  _______,  LOREM_ONE,  DMX_B1,   DMX_B2,
        KC_GRV,   KC_1,     KC_2,     KC_3,     KC_4,     KC_5,     KC_6,     KC_7,     KC_8,     KC_9,     KC_0,     KC_MINS,  KC_EQL,     KC_BSPC,    KC_INS,   KC_HOME,  KC_PGUP,  KC_NUM,   KC_PSLS,  KC_PAST,  KC_PMNS,
        KC_TAB,   KC_Q,     KC_W,     KC_E,     KC_R,     KC_T,     KC_Y,     KC_U,     KC_I,     KC_O,     KC_P,     KC_LBRC,  KC_RBRC,    KC_BSLS,    KC_DEL,   KC_END,   KC_PGDN,  KC_P7,    KC_P8,    KC_P9,
        KC_CAPS,  KC_A,     KC_S,     KC_D,     KC_F,     KC_G,     KC_H,     KC_J,     KC_K,     KC_L,     KC_SCLN,  KC_QUOT,              KC_ENT,                                   KC_P4,    KC_P5,    KC_P6,    KC_PPLS,
        KC_LSFT,            KC_Z,     KC_X,     KC_C,     KC_V,     KC_B,     KC_N,     KC_M,     KC_COMM,  KC_DOT,   KC_SLSH,              KC_RSFT,              KC_UP,              KC_P1,    KC_P2,    KC_P3,
...
uint8_t lorem_down = 0;
uint32_t lorem_timer = 0;
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        case DMX_B1:
        case DMX_B2:
            return dmx_process_record(keycode, record);
        case LOREM_ONE:
            if (record->event.pressed) {
                lorem_down = 1;
            } else {
                send_random_lorem_word(true);
                lorem_down = 0;
            }
            return false;
    }
...
void housekeeping_task_user(void) {
    if (lorem_down) {
        if (timer_elapsed(lorem_timer) > 50) {
            lorem_timer = timer_read();
            send_random_lorem_word(false);
        }
    }
...
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    if (dmx_rec_index != DMX_REC_INDEX_NONE) {
        for(int n = 0; n != 2; n++) {
            RGB_MATRIX_INDICATOR_SET_COLOR(DMX_LED(n), (n == dmx_rec_index) ? 255 : 0, 0, 0);
        }
    } else {
        for(int n = 0; n != 2; n++) {
            RGB_MATRIX_INDICATOR_SET_COLOR(DMX_LED(n), 0, 255, 0);
        }
    }

    if (is_caps_word_on()) {
        RGB_MATRIX_INDICATOR_SET_COLOR(79/*caps*/, 255, 255, 0);
    }

    RGB_MATRIX_INDICATOR_SET_COLOR(my_led, 255, 255, 255);
